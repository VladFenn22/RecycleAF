...
export function UpdateCupon() {
  ...
  useEffect(() => {
    if (id != undefined && !isNaN(Number(id))) {
      CuponService.getCuponFormById(Number(id))
        .then((response) => {
  ...
  const onSubmit = async (DataForm) => {
    ...
    try {
      ...
      if (cuponSchema.isValid()) {
        ...
        CuponServices.updateCupon(DataForm)
          .then((response) => {
            console.log(response);
  ...
  const fetchImage = async (imageName) => {
    try {
      const response = await CuponServices.getImage(imageName);
